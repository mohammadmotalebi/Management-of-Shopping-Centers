...
class usersRepositoryQueries implements usersRepositoryInterface
{
    public function usersActivationQuery($id, $limit = 0)
    {
        ...
        return DB::select("SELECT $top item_prices.item_price, item_credits.item_credit, item_scores.item_score, item_codes.item_code, bills.user_id, bills.buy_date, shops.id, shop_items.item_name, shops.shop_name, item_scores.id itemscoreid, SUM(CAST(item_scores.item_score AS INT)) totalscore, SUM(CAST(item_credits.item_credit AS INT)) totalcredit, SUM(CAST(item_prices.item_price AS INT)) total
FROM shop_items
...
WHERE        (shop_user.user_id = ?) and (shop_items.shop_id = ?) ORDER BY bills.buy_date DESC', [$id, $sid]);
    ...
    {
        return DB::select('SELECT        bills.bill_number,bills.status, bills.shop_id, bills.user_id, bills.buy_date, bill_items.item_id, shop_items.item_name, shop_items.price, item_codes.item_code, item_scores.credit, shops.shop_name
FROM            shop_items LEFT OUTER JOIN
                         item_codes ON shop_items.id = item_codes.item_id RIGHT OUTER JOIN
                         bill_items LEFT OUTER JOIN
                         bills ON bill_items.bill_id = bills.id ON item_codes.id = bill_items.item_id LEFT OUTER JOIN
                         shops ON shop_items.shop_id = shops.id FULL OUTER JOIN
                         item_scores ON shop_items.id = item_scores.item_id
                         where user_id = ?
                         ', [CurrentUserID()]);
    }
}
